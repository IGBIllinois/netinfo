...
$serialpath="/usr/local/bin/namespace/testserial.txt";
#$serialpath="serial.txt";

#directory where files are stored
#$directory='/var/named/chroot/var/named/';
$directory='/usr/local/bin/namespace/named/';
#$directory="named/";
...
while($row=$sth->fetchrow_hashref()){
  ...
  }elsif(${$row}{ipnumber}=~/172\.22\.87.(\d+)/){
    $inversenet.="$1\tIN PTR ${$row}{aname}$domain.\n";
  }else{
...
#db.87.22.172
open(OUTPUT, ">$directory/db.87.22.172");
print OUTPUT $dnsheader."\n\n".$addressheader.$inversenet;
close OUTPUT;
